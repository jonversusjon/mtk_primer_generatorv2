...
class SequencePreparator:
    ...
    def preprocess_sequence(
        ...
        send_update("Starting sequence preprocessing", 0)
        ...
        with logger.debug_context("pre_process_sequence"):
            ...
            send_update("Frame checked, looking for start/stop codons", 20)
            ...
            if matk_part_left in {"3", "3a"} and len(cleaned_sequence) >= 3 and cleaned_sequence[:3] == "ATG":
                ...
                send_update("Start codon detected and removed", 40)
            else:
                logger.log_step("Start Codon Check", "No start codon removal performed")
                send_update("No start codon detected or removal needed", 40)
            ...
            if len(cleaned_sequence) >= 3 and cleaned_sequence[-3:] in stop_codons:
                ...
                send_update("Stop codon detected and removed", 60)
            else:
                logger.log_step("Stop Codon Check", "No stop codon removal performed")
                send_update("No stop codon detected or removal needed", 60)
            ...
            if remainder != 0:
                if trim_start_codon:
                    ...
                    send_update(f"Adjusting frame by trimming {remainder} bases from the end", 80)
                elif trim_stop_codon:
                    logger.log_step("Frame Adjustment",
                                    f"Trimming {remainder} bases from the beginning due to frame remainder after stop codon removal")
                    cleaned_sequence = cleaned_sequence[remainder:]
                    send_update(f"Adjusting frame by trimming {remainder} bases from the beginning", 80)
                else:
                    logger.log_step("Frame Adjustment",
                                    f"Sequence length {final_length} is not a multiple of 3 and no codon removal detected",
                                    level=logging.WARNING)
                    send_update("Warning: Sequence not in frame and no codons to trim", 80)
            else:
                    send_update("Sequence already in frame, no adjustment needed", 80)
