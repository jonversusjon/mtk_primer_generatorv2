...
class MutationAnalyzer(DebugMixin):
    ...
    def __init__(self,
        ...
        if self.debug:
            ...
            self.validate(codon_usage_dict and
                          isinstance(codon_usage_dict, dict),
    ...
    def get_all_mutations(
        ...
        num_sites = len(sites_to_mutate) if sites_to_mutate else 0
        self.logger.debug(
            f"Entering get_all_mutations with {num_sites} sites to mutate."
        )
        
        self.validate(
            sites_to_mutate and isinstance(sites_to_mutate, list),
            f"Received {num_sites} sites to mutate",
            {"first_site": sites_to_mutate[0] if sites_to_mutate else None}
        )
        
        self.log_step(
            "Analysis Start",
            f"Starting mutation analysis for {num_sites} sites",
        ...
        try:
            with debug_context("mutation_analysis"):
                for site_idx, site in enumerate(sites_to_mutate):
                    ...
                    site_key = f"mutation_{pos}"
                    self.log_step(
                        "Process Site",
                        (f"Analyzing site {site_idx+1}/{len(sites_to_mutate)} at "
                        f"position {pos}"),
                        {"site_details": site}
                    )
                    ...
                    self.logger.debug(
                        f"Site {pos}: Found {len(codons_list)} codons for analysis."
                    )
                    for codon_idx, codon in enumerate(codons_list):
                        ...
                        if codon.get("codon_sequence") is None:
                            self.logger.warning(
                                f"Site {pos}: Codon {codon_idx+1} missing "
                                "codon_sequence. Skipping codon. Full codon data: "
                                f"{codon}"
                            )
                            continue
                        ...
                        self.log_step(
                            "Process Codon",
                            (f"Codon {codon_idx+1}: Processing sequence "
                            f"{codon.get('codon_sequence')}"),
                            {"codon_details": {
                        ...
                        site_with_indices = {
                            **site,
                            "overlapping_indices": self.utils.
                            get_recognition_site_bases(
                                site.get("frame"), codon_idx)
                        }
                        ...
                        if alternatives:
                            ...
                            self.log_step(
                                "Found Alternatives",
                                (f"Site {pos}, codon "
                                f"{codon.get('codon_sequence')}: Found "
                            ...
                            for alt in alternatives:
                                mutated_context, first_mut, last_mut = (
                                    self.get_mutated_context(
                                        context_sequence=site.get(
                                            "context_sequence"),
                                        codon_context_position=codon.get(
                                            "context_position"),
                                        new_codon_sequence=alt.get("seq"),
                                        mutated_context_mutated_bases=alt.get(
                                            "mutations")
                                    )
                                )
                                alt["mutated_context"] = mutated_context
                                alt["sticky_ends"] = (
                                    self._calculate_sticky_ends_with_context(
                                        mutated_context, first_mut, last_mut
                                    )
                                )
                                self.logger.debug(
                                    f"Alternative processed for codon "
                                    f"{codon.get('codon_sequence')}: {alt}"
                                )
                            ...
                            self.logger.debug(
                                f"Site {pos}, codon "
                                f"{codon.get('codon_sequence')}: No alternatives found."
                            )
                    ...
                    if site_has_alternatives:
                        ...
                        self.log_step(
                            "Site Result",
                            (f"Completed analysis for site at position {pos}"),
                            {"alternatives_found": sum(
                                len(c.get("alternative_codons", []))
                                for c in site_mutations["codons"]
                            ),
                        ...
                        self.log_step(
                            ...
                            f"Site {pos}: No alternative codons found",
                            {"site": pos},
                            level=logging.WARNING
                        )
                        self.logger.debug(
                            f"Site {pos}: Mutation analysis skipped due to absence "
                            "of alternatives."
                        )
            ...
            self.logger.error(
                f"Critical error in mutation analysis: {e}",
                exc_info=True
            )
            raise e
    ...
    @DebugMixin.debug_wrapper
    def _find_alternative_codons(self, site: Dict, original_codon: str,
                                amino_acid: str, codon_context_position: int
                                ) -> List[Dict]:
        ...
        self.log_step(
            "Possible Codons",
            (f"Found {len(alt_codons)} synonymous codons for amino acid "
            f"{amino_acid}"),
            {"codons": alt_codons}
        )
        ...
        for cand in alt_codons:
            ...
            self.log_step(
                ...
                (f"Candidate {cand} differs from {original_codon} at "
                f"positions {muts}"),
                {"candidate": cand, "original": original_codon, "differences": muts},
                level=logging.DEBUG
            )
            ...
            if len(muts) > self.max_mutations:
                self.log_step(
                    "Candidate Skipped",
                    (f"Candidate {cand} has {len(muts)} mutations "
                    f"(allowed: {self.max_mutations})."),
                    {"mutations": muts},
                    level=logging.DEBUG
                )
                continue
            ...
            self.log_step(
                "Mutation Positions",
                (f"Candidate {cand} mutation positions in context: "
                f"{mutation_positions_in_context}"),
                {"mutations": muts,
                "codon_context_position": codon_context_position},
                level=logging.DEBUG
            )

            # Determine which of these mutation positions are within the
            # recognition (overlapping) region
            changes = [
                i for i in muts
                if i in site.get("overlapping_indices", [])
            ]
            self.log_step(
                "Overlap Check",
                (f"Candidate {cand} changes within recognition region: "
                f"{changes}"),
                {"overlapping_indices": site.get("overlapping_indices"),
                "mutations": muts},
                level=logging.DEBUG
            ...
            # If no changes occur in the overlapping (recognition) region,
            # skip candidate
            if not changes:
                self.log_step(
                    "Candidate Skipped",
            ...
            # Build a mutation tuple representing which positions are mutated (1)
            # or not (0)
            mut_tuple = tuple(
        ...
        self.log_step(
            "Find Alternatives Complete",
            (f"Total valid alternatives for codon {original_codon}: "
            f"{len(valid)}"),
    ...
    def _calculate_sticky_ends_with_context(self, mutated_ctx: str,
        ...
        sticky = {}
        preview = mutated_ctx[:50] + "..."
        self.log_step(
            "Calculate Sticky Ends",
            (f"Calculating sticky ends for boundaries: "
            f"first_mut_idx={first_mut_idx}, last_mut_idx={last_mut_idx}"),
            {"mutation_context_preview": preview,
    ...
    @DebugMixin.debug_wrapper
    def get_mutated_context(self, context_sequence: str, codon_context_position: int,
                            new_codon_sequence: str,
                            mutated_context_mutated_bases: tuple) -> tuple:
        self.log_step(
            "get_mutated_context",
            (f"Starting codon swap at position {codon_context_position} with "
            f"new codon '{new_codon_sequence}'")
        )
        if len(new_codon_sequence) != 3:
            raise ValueError("New codon must be 3 nucleotides long.")
        if (codon_context_position < 0 or
                codon_context_position + 3 > len(context_sequence)):
            raise ValueError(
                "Invalid codon_context_position or context_sequence too short for the "
                "swap."
            )
        ...
        try:
            ...
            last_mutation_index = (len(mutated_context_mutated_bases) - 1 -
                                mutated_context_mutated_bases[::-1].index(1))
        except ValueError:
            first_mutation_index = None
            last_mutation_index = None
        self.validate(
            isinstance(first_mutation_index, int),
        ...
        self.validate(
            ...
            (first_mutation_index != last_mutation_index and num_mutations > 1)),
            "Mutation indices do not match the expected number of mutated bases"
        )
        self.log_step(
            "get_mutated_context", "Codon swap complete",
        ...
        mutated_context_first_mutation_index = (
            first_mutation_index + codon_context_position
        )
        mutated_context_last_mutation_index = (
            last_mutation_index + codon_context_position
        )
        return (mutated_context, mutated_context_first_mutation_index,
                mutated_context_last_mutation_index)


    @DebugMixin.debug_wrapper
    def _create_mutation_entry(self, site_sequence: str, site_start: int,
                            codon_start: int, codon_pos: int,
                            original_codon: str, new_codon: str,
                            frequency: float, enzyme: str, strand: str
                            ) -> Optional[Dict]:
        self.log_step(
            "Create Mutation Entry",
            (f"Creating mutation entry for {original_codon} → {new_codon}"),
            {"site_sequence": site_sequence, "site_start": site_start,
            "codon_start": codon_start, "codon_pos": codon_pos,
            "frequency": frequency, "enzyme": enzyme, "strand": strand}
        )
        ...
        self.log_step(
            ...
            "changed_base_pos": codon_pos, "original_base": original_base,
            "new_base": new_base}
        )
        if mutated_site == site_sequence:
            if getattr(self, 'debugger', None):
                self.debugger.log_warning(
                    (f"Mutation does not disrupt restriction site: {original_base} "
                    f"→ {new_base} at position {mutation_position}")
                )
            return None
        entry = {
