 ...
 */
export const validateDnaSequence = (sequence, isRequired = false) => {
    ...
    if (!validDnaRegex.test(cleanedSequence)) {
      return {
        isValid: false,
        message: 'Only valid DNA bases (A, T, G, C) or extended DNA code (W, S, M, K, R, Y, B, D, H, V, N) allowed',
        isAdvisory: false
      };
    }
    ...
    // Check if sequence is in frame (divisible by 3)
    ...
    if (hasStartCodon && hasStopCodon) {
      return {
        isValid: true,
        message: 'Start and stop codons detected and will be removed during processing',
        isAdvisory: true
      };
